...
def kumaraswamy_inv_cdf(u, a, b):
    """
    Inversa chiusa: F^{-1}(u) = (1 - (1-u)^{1/b})^{1/a}
    """
    u = np.asarray(u)
...
def kumaraswamy_equiprobable_points(a, b, n_points):
    """
    Genera punti equiprobabili tramite l'inversa chiusa.
...
def kumaraswamy_log_pdf(x, a=3.0, b=3.0):
    """
    log pdf di Kumaraswamy(a,b) su (0,1).
    Ignora costante di normalizzazione, compatibile con ARS.
    """
    x = np.asarray(x)
    # maschera per (0,1)
    eps = 1e-15
...
def kumaraswamy_log_pdf_prime(x, a=3.0, b=3.0):
    """
    Derivata rispetto a x della log-pdf di Kumaraswamy(a,b).
    """
    x = np.asarray(x)
    eps = 1e-15
    x_clipped = np.clip(x, eps, 1.0 - eps)
